...
def main(
    ckpt_path: str = "checkpoints/mlp-conv/latest/epoch_040.pt",
    data_dir: str = "data/raw",
    traj_for_latent: int | None = None,
):
    ...
    model = MLP_E2C(x_dim, u_dim)
    model.load_state_dict(state_dict)
    model.eval()
    device = "cpu"
    model.to(device)

    print(f"[viz-mlp] frames.shape  = {frames.shape}")
    print(f"[viz-mlp] actions.shape = {actions.shape}")
...
def plot_latent_trajectory(model, frames, traj_id=None, device="cpu"):
    ...
    print(f"[viz-mlp] Plotting latent trajectory for traj_id={traj_id}")

    Z_list = []
    with torch.no_grad():
        for t in range(T):
            x_tensor = torch.from_numpy(frames[traj_id, t:t+1, 0]).float().to(device)  # (1,1,H,W)
            mu, logvar = model.vae.encoder(x_tensor)  # (1, z_dim)
            mu_np = mu.cpu().numpy().squeeze(0)       # (z_dim,)
            Z_list.append(mu_np)

    Z = np.stack(Z_list, axis=0)  # (T, z_dim)
    z_dim = Z.shape[1]
    ...
    if z_dim >= 2:
        sc = axs[0].scatter(Z[:, 0], Z[:, 1], c=steps, cmap="viridis", s=5)
        ...
        axs[0].plot(steps, Z[:, 0], "-o", markersize=2)
        axs[0].set_title("Latent z over time")
        axs[0].set_xlabel("t")
        axs[0].set_ylabel("z")

    for d in range(z_dim):
        axs[1].plot(steps, Z[:, d], label=f"z[{d}]")
...
def plot_rollout_mujoco(
    ...
    make_gif: bool = False,
    gif_path: str = "rollout_mlp_gt_pred.gif",
    gif_fps: int = 4,
    ...
    S = getattr(e2c_config, "state_seq_len", 1)

    t0 = max(S - 1, min(t0, T - 1))
    max_horizon = max(0, T - 1 - t0)
    if horizon is None or horizon > max_horizon:
        horizon = max_horizon

    print(f"[viz-mlp] Rollout: traj_id={traj_id}, t0={t0}, horizon={horizon}")
    ...
    with torch.no_grad():
        ...
        for k in range(horizon):
            ...
            A_t, B_t, o_t, *_ = model.transition(z_t)  # shapes: (1,d,d), (1,d,A), (1,d)
            z_tp1 = model.transition.forward_dynamics(A_t, B_t, o_t, z_t, u_t)

            x_pred_tp1, _ = model.vae.decoder(z_tp1)  # (1,1,H,W)
            pred_arr = x_pred_tp1.squeeze(0).cpu().numpy()  # (S,H,W) or (H,W)
            x_pred_img = pred_arr[-1] if pred_arr.ndim == 3 else pred_arr  # (H, W)
    ...
    # precompute rollout lengths for plots
    T_vis = horizon + 1
    T_pic = min(10, T_vis)

    if show_plots or save_plot_path:
        fig, axs = plt.subplots(2, T_pic, figsize=(1.8 * T_pic, 4))
        ...
        plt.suptitle(f"MLP_E2C rollout (traj={traj_id}, start={t0}, shown={T_pic-1})")
        plt.tight_layout()
        if save_plot_path:
    ...
    if make_gif:
        ...
        print(f"[viz-mlp] Saved GIF to {gif_path}")


if __name__ == "__main__":
    main(
        ckpt_path="checkpoints/mlp-conv/latest/epoch_040.pt",
        data_dir="data/raw",
        traj_for_latent=None,
    )
